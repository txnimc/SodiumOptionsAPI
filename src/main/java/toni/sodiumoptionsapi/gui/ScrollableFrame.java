...
public class ScrollableFrame extends AbstractFrame {
    ...
    public void setupFrame(AtomicReference<Integer> verticalScrollBarOffset, AtomicReference<Integer> horizontalScrollBarOffset) {
        int maxWidth = 0;
        int maxHeight = 0;
        if (!(#if !FORGE (Dim2iExtended) (Object) #endif this.dim).canFitDimension(this.frame.dim)) {
        ...
        if (this.canScrollHorizontal && this.canScrollVertical) {
            ...
            this.frame.dim = withHeight(this.frame.dim, this.frame.dim.height() - 11); // FIXME don't mutate
        } else if (this.canScrollVertical) {
            this.viewPortDimension = new Dim2i(this.dim.x(), this.dim.y(), this.dim.width() - 11, this.dim.height());
            this.frame.dim = withWidth(this.frame.dim, this.frame.dim.width() - 11); // FIXME don't mutate
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double verticalAmount) {
        return (this.canScrollHorizontal && this.horizontalScrollBar.mouseScrolled(mouseX, mouseY, scrollX, verticalAmount)) || (this.canScrollVertical && this.verticalScrollBar.mouseScrolled(mouseX, mouseY, scrollX, verticalAmount)) || super.mouseScrolled(applyOffset(this.horizontalScrollBar, mouseX, false), applyOffset(this.verticalScrollBar, mouseY, false), scrollX, verticalAmount);
    }
    #else
    @Override
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX) {
        return (this.canScrollHorizontal && this.horizontalScrollBar.mouseScrolled(mouseX, mouseY, scrollX)) || (this.canScrollVertical && this.verticalScrollBar.mouseScrolled(mouseX, mouseY, scrollX)) || super.mouseScrolled(applyOffset(this.horizontalScrollBar, mouseX, false), applyOffset(this.verticalScrollBar, mouseY, false), scrollX);
    }
    #endif
